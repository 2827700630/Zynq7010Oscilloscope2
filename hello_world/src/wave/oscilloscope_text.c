...
#include "wave.h"
#include "../adc_dma_ctrl.h" // 添加头文件以支持TimebaseConfig
#include <string.h>
#include <stdio.h>
#include <math.h>
...
void draw_oscilloscope_info(u8 *canvas, u32 canvas_width, u32 canvas_height, OscilloscopeParams *params)
{
    ...
    /* 触发信息 */
    draw_string(canvas, canvas_width, x, y, "Trig: ", TEXT_COLOR_CYAN);
    draw_number_float(canvas, canvas_width, x + 6 * FONT_WIDTH, y, params->trigger_voltage, 2, TEXT_COLOR_WHITE);
    draw_string(canvas, canvas_width, x + 12 * FONT_WIDTH, y, "V ", TEXT_COLOR_CYAN);

    // 触发状态指示
    switch (params->trigger_status)
    {
    case TRIGGER_STATUS_TRIGGERED:
        draw_string(canvas, canvas_width, x + 15 * FONT_WIDTH, y, "TRIG", TEXT_COLOR_GREEN);
        break;
    case TRIGGER_STATUS_WAITING:
        draw_string(canvas, canvas_width, x + 15 * FONT_WIDTH, y, "WAIT", TEXT_COLOR_YELLOW);
        break;
    case TRIGGER_STATUS_TIMEOUT:
        draw_string(canvas, canvas_width, x + 15 * FONT_WIDTH, y, "AUTO", TEXT_COLOR_RED);
        break;
    }
}
...
{
    // 计算触发电平对应的Y坐标
    // ADC码值范围：0-255，对应屏幕Y坐标：canvas_height到0
    u32 trigger_y = canvas_height - (params->trigger_level * canvas_height / 255);

    // 限制在有效范围内
    if (trigger_y >= canvas_height)
        trigger_y = canvas_height - 1;

    // 绘制水平触发线（红色虚线）
    for (u32 x = 0; x < canvas_width; x += 10)
    { // 每10像素绘制5像素，形成虚线效果
        for (u32 i = 0; i < 5 && (x + i) < canvas_width; i++)
        {
            draw_point(canvas, x + i, trigger_y, canvas_width, 0, 0, 255); // 红色
        }
    }

    // 在触发线左侧绘制触发标记 "T"
    if (canvas_width > 20 && trigger_y > FONT_HEIGHT)
    {
        draw_char(canvas, canvas_width, 5, trigger_y - FONT_HEIGHT / 2, 'T', TEXT_COLOR_RED);
    }
}
